...
/**
 * `response.write()` returns false once the socket buffer is full. Ignoring it
 * queues the entire upstream stream in memory whenever the client reads slower
 * than Anthropic sends, which is the normal case for long SSE generations.
 * `pipeline` applies backpressure and propagates client aborts and errors.
 */
...
/**
 * @param {{
 *   config: typeof import('./config.mjs').CONFIG,
 *   resolveAccount: (secret: string) => (import('./accounts.mjs').Account | null),
...
export async function startProxyServer({ config, resolveAccount, log = () => {} }) {
	const server = http.createServer(async (request, response) => {
		...
		try {
			...
			if (!response.headersSent) {
				// Oversized bodies report 413 instead of a misleading upstream 502.
	...
	/** Fallback socket tracking for runtimes without `closeAllConnections()`. */
	...
	return {
		...
		server,
		/**
		 * `server.close()` alone waits for every keep-alive socket to go idle, so a
		 * single parked SillyTavern connection would hang shutdown forever. Drop
		 * idle sockets immediately and force the rest after a short grace period so
		 * an in-flight generation still gets a chance to finish.
		 */
		close: async () => {
			const closed = /** @type {Promise<void>} */ (new Promise((resolve) => {
