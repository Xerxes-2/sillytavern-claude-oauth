 ...
 *
 * Every request must carry `proxySecret` as `x-api-key` (ST's "proxy password")
 * so other local processes cannot spend the subscription.
 */
export async function startProxyServer({ config, proxySecret, getAccessToken, log = () => {} }) {
	if (!proxySecret) {
		throw new Error('startProxyServer requires a proxySecret');
	}
	...
	const server = http.createServer(async (request, response) => {
		...
		if (!secretMatches(presentedSecret(request.headers), proxySecret)) {
			request.resume();
			response.writeHead(401, { 'content-type': 'application/json' });
			response.end(JSON.stringify({
				error: { type: 'authentication_error', message: 'Wrong Claude OAuth proxy password. Click "Auto-configure" in the extension panel, or copy proxySecret from /api/plugins/claude-oauth/status into the proxy password field.' },
			}));
			return;
		}
		...
		try {
			...
			log(`Claude OAuth proxy error: ${error.message}`);
