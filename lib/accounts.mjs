...
export function validateAccountName(name) {
  ...
  if (!NAME_RE.test(value)) {
    throw fail('invalid_account_name', 'Account name must be 1-32 characters: letters, digits, "-" or "_".')
  }
  return value
}
...
/**
 * Write 0600 + fsync into a temp file, then rename over the target. The fsync
 * matters: without it a crash (or a pulled plug) can land the rename before the
 * data, leaving an empty credential file and a logged-out account.
 */
...
export function createAccountRegistry({ dataRoot, refresh, log = () => {} }) {
  ...
  function makeAccount({ handle, name, secret, createdAt, credentials, file }) {
    /** @type {Promise<Credentials>|null} In-flight refresh, shared by concurrent callers. */
    let inflight = null
    ...
    const account = {
      ...
      async save(next) {
        if (!validCredentials(next)) {
          throw fail('invalid_credentials', 'Refusing to save incomplete OAuth credentials.', 500)
        }
      ...
      async getAccessToken() {
        ...
        if (!validCredentials(creds)) {
          throw fail('account_not_logged_in', `Claude account "${name}" has no credentials. Log in again from the plugin panel.`)
        }
        ...
        try {
          ...
          throw new Error(`Claude OAuth token refresh failed for ${account.label}: ${error.message}`, { cause: error })
        }
      },
    }
    return account
 ...
 * @property {(creds: Credentials) => Promise<void>} save
 * @property {() => Promise<string>} getAccessToken
 * @property {(usage: object) => void} recordUsage
 */
