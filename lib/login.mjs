import { deferred, delay, fail } from './util.mjs'
...
export function createLoginManager({ oauth, config, onSuccess, log = () => {} }) {
  ...
  function manualCodeChannel(state) {
    return signal =>
      new Promise((resolve, reject) => {
        ...
        const onAbort = () => state.rejectCode?.(fail('manual_code_not_needed', 'Manual code entry no longer needed.'))
        ...
        state.timer = setTimeout(() => {
          state.rejectCode?.(fail('login_timeout', 'Claude login timed out. Start the login again.', 408))
  ...
  function own(owner) {
    ...
    if (pending.owner !== owner) {
      throw fail('login_busy', 'Another Claude login is in progress on this server. Wait for it to finish or time out.', 409)
    }
    return pending
  }
  ...
  async function start({ owner, name }) {
    ...
    if (existing) {
      if (existing.name !== name) {
        throw fail('login_other_account', `A login for account "${existing.name}" is already in progress. Cancel it first.`, 409)
      }
    ...
    if (!state.authUrl) {
      throw fail('login_failed', state.error || 'Claude OAuth login did not produce an authorization URL.', 502)
    }
  ...
  async function submitCode(owner, input) {
    ...
    if (!state?.resolveCode) {
      throw fail('no_login_in_progress', 'No Claude login is in progress.')
    }
    const value = String(input || '').trim()
    if (!value) {
      throw fail('missing_code', 'Paste the full redirect URL (or the authorization code) first.')
    }
    state.resolveCode(value)
    await state.promise
    if (state.error) {
  ...
  function cancel(owner) {
    ...
    state.rejectCode?.(fail('login_cancelled', 'Claude login cancelled.'))
    state.abort.abort(fail('login_cancelled', 'Claude login cancelled.'))
