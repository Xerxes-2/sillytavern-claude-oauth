...
export const CONFIG = {
  ...
  /**
   * Sent as `user-agent: claude-cli/<version>`; OAuth requests are rejected
   * without CLI-shaped headers. Anthropic also gates new models on this
   * version ("Claude Code X does not support this model; version Y or newer is
   * required", error_code claude_code_version_too_old). Bump to the current
   * `npm view @anthropic-ai/claude-code version` when that appears.
   */
  claudeCodeVersion: process.env.CLAUDE_OAUTH_CLI_VERSION || '2.1.280',
  ...
  /** Grace period for in-flight proxy responses on shutdown before sockets are forced closed. */
  shutdownGraceMs: Number(process.env.CLAUDE_OAUTH_SHUTDOWN_GRACE_MS || 2000),
}
